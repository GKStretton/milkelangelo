...
def buildDslr(n_str: str, unlisted: bool = False) -> pb.ContentTypeStatus:
    ct = pb.ContentType.CONTENT_TYPE_DSLR
    ...
    s.posts.append(pb.Post(
        platform=platform,
        title=append_title_hashtags(f"{n_str} (dslr): {s.raw_title}", ct, platform),
    ...
    # s.posts.append(pb.Post(
    #     platform=platform,
    #     title=f"{n_str} (dslr): {s.raw_title}\n\n{s.raw_description}\n\n{get_common_text(ct, platform)}\n\n{get_hashtags(ct, platform)}",
    #     description="N/A",
    #     crosspost=False,
    #     scheduled_unix_timetamp=get_schedule_timestamp(ct),
    #     unlisted=unlisted,
